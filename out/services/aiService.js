...
class AIService {
    ...
    static async readFileContents(files) {
    ...
    static getConfigMessage(files) {
        const configFiles = files.filter(f => f.toLowerCase().includes('config') ||
            f.endsWith('.json') ||
            f.endsWith('.yaml') ||
            f.endsWith('.yml'));
    ...
    static getCodeMessage(files, analysis) {
        // Try to infer what functionality was updated
        const functions = analysis.functions.filter((f) => f.length > 0);
    ...
    static humanizeFileName(fileName) {
    ...
    static getDetailedDescription(files, analysis) {
        const lines = ['Changes:'];
        if (analysis.hasCode) {
