...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('gitAutoCommit.generateCommit', () => generateAndCommit(context)),
        vscode.commands.registerCommand('gitAutoCommit.showDashboard', () => showDashboard(context)),
        vscode.commands.registerCommand('gitAutoCommit.toggleAutoCommit', () => toggleAutoCommit(context)),
        vscode.commands.registerCommand('gitAutoCommit.configureAI', () => configureAI(context))
    );
...
async function showWelcomeAndRequestPermission(context: vscode.ExtensionContext) {
...
function getApiKeySetupInstructions(provider: string): string {
...
function startAutoCommitTimer(context: vscode.ExtensionContext) {
    stopAutoCommitTimer();
    const config = vscode.workspace.getConfiguration('gitAutoCommit');
...
function startReminderTimer(context: vscode.ExtensionContext) {
    stopReminderTimer();
    const config = vscode.workspace.getConfiguration('gitAutoCommit');
...
async function generateAICommitMessage(files: string[], diff: string): Promise<string> {
    const config = vscode.workspace.getConfiguration('gitAutoCommit');
    const provider = config.get('aiProvider', 'openai') as string;
...
async function generateAndCommit(context: vscode.ExtensionContext, silent: boolean = false) {
    try {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            if (!silent) {
...
async function commitChanges(context: vscode.ExtensionContext, message: string, files: string[], cwd: string) {
    try {
